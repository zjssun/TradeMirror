...
import { TradeReplayPage } from "../features/market/TradeReplayPage";
import { TradeListPage } from "../features/trades/TradeListPage";
import { I18nProvider, useI18n, type Language } from "./i18n";

const { Header, Content } = Layout;
type View = "dashboard" | "datasource" | "import" | "trades" | "replay" | "prompt" | "export";
      ...
      <Layout style={{ minHeight: "100vh" }}>
        <Header style={{ display: "flex", alignItems: "center", gap: 40, background: "#001529" }}>
          ...
          <Menu theme="dark" mode="horizontal" selectedKeys={[view]} onClick={({ key }) => setView(key as View)} items={[
            { key: "dashboard", label: t("nav.dashboard") }, { key: "datasource", label: t("nav.datasource") }, { key: "prompt", label: t("nav.narrative") }, { key: "trades", label: t("nav.trades") }, { key: "replay", label: t("nav.replay") }, { key: "import", label: t("nav.import") }, { key: "export", label: t("nav.export") },
        ...
        <Content style={{ padding: 32, maxWidth: 1280, width: "100%", margin: "0 auto" }}>
          {view === "dashboard" && <DashboardPage />}{view === "datasource" && <DataSourcePage />}{view === "import" && <TradeImportPage />}{view === "trades" && <TradeListPage />}{view === "replay" && <TradeReplayPage />}{view === "prompt" && <AiPromptPage />}{view === "export" && <TmfExportPage />}
        </Content>
      </Layout>
    </AntApp>
  </ConfigProvider>;
