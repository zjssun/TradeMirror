...
interface Props { candles: MarketCandle[]; direction?: string; entryPrice?: number; exitPrice?: number; stopLoss?: number | null; takeProfit?: number | null; entryTime?: string; exitTime?: string; }
...
export function MarketChart({ candles, direction, entryPrice, exitPrice, stopLoss, takeProfit, entryTime, exitTime }: Props) {
  ...
  useEffect(() => {
    ...
    const entryMarkerTime = entryTime && candleTimeAtOrBefore(candles, entryTime); const exitMarkerTime = exitTime && candleTimeAtOrBefore(candles, exitTime); const isBuy = direction === "BUY"; const markers: SeriesMarker<UTCTimestamp>[] = [];
    if (entryMarkerTime && typeof entryPrice === "number") markers.push({ time: entryMarkerTime, position: isBuy ? "atPriceBottom" : "atPriceTop", price: entryPrice, color: "#2563eb", shape: isBuy ? "arrowUp" : "arrowDown", text: isBuy ? t("market.openBuy") : t("market.openSell"), size: 2 });
    if (exitMarkerTime && typeof exitPrice === "number") markers.push({ time: exitMarkerTime, position: isBuy ? "atPriceTop" : "atPriceBottom", price: exitPrice, color: "#7c3aed", shape: isBuy ? "arrowDown" : "arrowUp", text: isBuy ? t("market.closeBuy") : t("market.closeSell"), size: 2 });
    ...
    chart.timeScale().fitContent(); return () => { chartRef.current = null; chart.remove(); };
  }, [candles, direction, entryPrice, exitPrice, stopLoss, takeProfit, entryTime, exitTime, t]);
